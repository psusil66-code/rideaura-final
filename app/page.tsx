...
export default function Home(){
  ...
  return <main>
      ...
      <div className="container hero-stage">
        <div className="hero-copy">
          ...
          <div className="hero-actions">
            ...
            <a className="btn ghost" href="/cars">Car Rent</a>
            <a className="btn ghost" href="/bikes">Bikes</a>
          </div>
        ...
        <form className="hero-search" id="booking">
          ...
          <a className="btn search-btn" href="/cars">Search Cars</a>
        </form>
    ...
    <section className="section" id="fleet"><div className="container"><div className="title"><span className="eyebrow">Our Fleet</span><h2>Choose cars or bikes separately</h2><p>Open the car page for self-drive cars, or the bike page for bike and scooter rentals.</p><div className="hero-actions fleet-page-actions"><a className="btn" href="/cars">Car Rent</a><a className="btn dark" href="/bikes">Bikes</a></div></div>{loadingCars ? <div className="panel">Loading Ride Aura vehicles...</div> : <div className="grid3">{cars.slice(0,6).map(car=><article className="card fleet-card" key={car.id}>{car.image_url ? <img src={car.image_url} alt={car.name}/> : <div className="booking-car-empty">No Image</div>}<div className="card-body"><h3>{car.name}</h3><div className="meta"><span>{car.seats} {isBike(car) ? 'Riders' : 'Seats'}</span><span>{car.fuel_type}</span><span>{car.transmission}</span><span>{car.status}</span></div><div className="price-row"><div className="price-box"><small>Per Day</small><div className="price">Rs. {car.price_per_day}</div></div><div className="price-box"><small>Per Hour</small><div className="price">Rs. {car.price_per_hour}</div></div></div><a className="btn" href={isBike(car) ? `/booking?vehicle=${encodeURIComponent(car.id)}&type=bike` : `/booking?vehicle=${encodeURIComponent(car.id)}&type=car`} style={{width:'100%'}}>Check Availability</a></div></article>)}{cars.length === 0 && <div className="panel">No vehicles added yet. Add cars or bikes from the admin dashboard to show them here.</div>}</div>}</div></section>
    ...
    <section className="section local-search-section"><div className="container"><div className="title"><span className="eyebrow">Ride Aura Bhubaneswar</span><h2>Car and bike self-drive rental service near you</h2><p>Ride Aura Self Drive is built for customers searching for the best car rental service in Bhubaneswar, best bike rental service in Bhubaneswar, self drive near me, self drive nearby, self drive near airport, self drive near railway station, best self drive near airport, car self drive in Bhubaneswar and bike self drive service in Bhubaneswar. Book cars, bikes and scooters for city use, Bhubaneswar Airport pickup, Bhubaneswar Railway Station pickup, Puri, Konark, Chilika and Odisha road trips.</p></div></div></section>
