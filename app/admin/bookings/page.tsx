...
export default function Bookings() {
  ...
  return <main className="admin-page"><div className="admin-page-head"><div><h1>Bookings</h1><p>View booking requests and update status: Pending, Confirmed, Rejected, Completed.</p></div><button className="btn dark" onClick={signOut}>Logout</button></div>{message && <p className="admin-notice">{message}</p>}<div className="admin-table-wrap"><table className="table"><thead><tr><th>ID</th><th>Name</th><th>Phone</th><th>Car</th><th>Pickup</th><th>Return</th><th>Location</th><th>License</th><th>Status</th></tr></thead><tbody>{bookings.map(b=><tr key={b.id}><td>{b.id.slice(0,8)}</td><td>{b.customer_name}</td><td><a href={`tel:${b.phone}`}>{b.phone}</a></td><td>{b.car_id || '-'}</td><td>{b.pickup_at ? new Date(b.pickup_at).toLocaleString() : '-'}</td><td>{b.return_at ? new Date(b.return_at).toLocaleString() : '-'}</td><td>{b.location || '-'}</td><td>{b.license_path ? <button type="button" onClick={()=>getLicenseUrl(b.license_path)}>View</button> : '-'}</td><td><select value={b.status || 'Pending'} onChange={(e)=>updateStatus(b.id, e.target.value)}>{statuses.map(status=><option key={status}>{status}</option>)}</select></td></tr>)}{bookings.length === 0 && <tr><td colSpan={9}>No bookings yet.</td></tr>}</tbody></table></div></main>;
}
